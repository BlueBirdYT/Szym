...
async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='for Rhelp', type=2))
...
async def on_ready():
     ...
     print('.......')
     print('created by syzm and bluebird')
     client.loop.create_task(status_task())

def is_owner(ctx):
    return ctx.message.author.id in ["455322915471097857","422016401704878091"]
...
async def getuser(ctx, role: discord.Role = None):
    if role is None:
        await client.say('Please tag a role to get users having it. Example- ``Rgetuser @role``')
        return
    if ctx.message.author.server_permissions.kick_members == False:
       await client.say('**You do not have permission to use this command**')
       return
...
async def userinfo(ctx, user: discord.Member=None):
    if user is None:
      await client.say('Please tag a user to get user information. Example- ``Ruserinfo @user``')
    if ctx.message.author.bot:
      return
    if ctx.message.author.server_permissions.kick_members == False:
      await client.say('**You do not have permission to use this command**')
...
async def rolecolor(ctx, role:discord.Role=None, value:str=None):
    if discord.utils.get(ctx.message.server.roles, name="{}".format(role)) is None:
        await client.say("Use this command like ``Rrolecolor (ROLENAME) (ROLECOLOUR IN HEXCODE)``")
        return
    if value is None:
        await client.say("Use this command like ``Rrolecolor (ROLENAME) (ROLECOLOUR IN HEXCODE)``")
        return
...
async def warn(ctx, userName: discord.User=None,*, message:str=None):
    if userName is None:
      await client.say('Please tag a person to warn user. Example- ``Rwarn @user <reason>``')
      return
    else:
...
async def mute(ctx, member: discord.Member=None, mutetime=None):
    ...
    if member is None:
        await client.say('Please specify member i.e. Mention a member to mute. Example-``Rmute @user <time in minutes>``')
        return
    if mutetime is None:
...
async def avatar(ctx, user: discord.Member=None):
    if user is None:
        ...
        embed.set_thumbnail(url='https://cdn.discordapp.com/avatars/486588730845560833/0f8165d281769b680a810e8c24ba2568.webp?size=1024')
...
async def access(ctx, member: discord.Member=None):
    if member is None:
      await client.say("Please specify a member to give access to him. Example- ``Raccess @user``")
    if ctx.message.author.bot:
      return
    if ctx.message.author.server_permissions.kick_members == False:
      await client.say('**You do not have permission to use this command**')
...
async def setnick(ctx, user: discord.Member=None, *, nickname=None):
    if user is None:
      await client.say('Please tag a person to change nickname. Example- ``Rsetnick @user <new nickname>``')
      return
    else:
...
async def ban(ctx,user:discord.Member=None):
    if user is None:
      await client.say('Please specify a member to ban. Example- ``Rban @user``')
